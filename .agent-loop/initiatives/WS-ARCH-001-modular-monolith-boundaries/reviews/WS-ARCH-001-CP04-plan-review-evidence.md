...
historical — not current approval evidence

This record describes the earlier planning review at `445944a`. Main was later
merged into the PR branch, producing `769e15e` and invalidating exact-head
freshness. It must not be used as current reviewer approval. Fresh reviewer
receipts remain private advisory-session evidence under the upgraded protocol;
the PR body may mirror their verdicts only when it names the exact reviewed
head.
...
## Historical reviewer results

| Track | Historical result at `445944a` | Remaining blocker at that head |
|---|---|---|
...
These historical results do not authorize merge. This planning change adds no
runtime code, migration, action activation,
