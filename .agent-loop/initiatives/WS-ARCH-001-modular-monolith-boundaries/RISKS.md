...
| Legacy payment/compensation policy vocabulary becomes a second governing model | Critical | ContributionPolicyVersion is the only frozen award-governing policy; remove legacy payment-policy fields and names through owner clean cuts before public release, with no aliases or fallback |
| Live assignment or review claim occurs without its actor-specific frozen ContributionPolicyVersion | Critical | CON selection/freeze participates atomically in TaskAssignment and ReviewLease creation; missing or invalid versions deny with no row created |
