...
| Policy mutation consumes AUTH before an operation race is fenced | Critical | CP04A installs one transaction-scoped operation fence and immutable recovery event used by every CP04 mutation; recovery requires current read authorization and creates no second effect/evidence |
| Publication authorizes caller-supplied economic content | Critical | CP04B locks the complete graph and referenced unit/binding rows, recomputes canonical facts from server-owned state, and consumes PREP before mutation |
| Draft update races publication or leaves a partial graph | Critical | CP04A replaces the complete two-rule graph atomically; CP04B holds the same custody through publication and database guards reject incomplete/mixed state |
| Retirement rewrites active work or historical awards | Critical | Retirement disables future selection only; frozen guide/task/assignment/Submission/ReviewLease/contribution/award lineage remains immutable |
