...
| Human review | `PreSubmitCheckerPolicy` must be persisted and locked, not derived on read. | High | Fixed | Updated plan, ADRs, data model, lifecycle, checker flow, and chunk contracts to require persisted project checker snapshot/hash. Tasks lock the project checker hash; they do not compile their own checker. |
...
| Human review | Prior edits overcorrected into task-level checker generation. | High | Fixed | Removed per-task policy/checker generation from the plan. Chunk 2 persists the project `PreSubmitCheckerPolicy`; Chunk 3 only locks task references to the project guide, effective policy, and checker hash. |
