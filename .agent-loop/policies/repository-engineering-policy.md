...
| Signed-state drift detection | `.github/workflows/loop-memory-drift-audit.yml` | A scheduled or protected-default-branch manual audit independently verifies signed custody, semantics, ancestry, and active contract binding with read-only permissions. It reports advancement separately from corruption and has no repair, signing, dispatch, or publication capability. |
