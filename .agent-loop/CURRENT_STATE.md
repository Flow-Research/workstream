...
|---|---|---|
| [WS-ARCH-001](initiatives/WS-ARCH-001-modular-monolith-boundaries/STATUS.md) | Boundary foundation is merged through PR #310; no product behavior changed | Review the proposed submission-capability split across TASKS, PROJECTS, CHECKERS, ART, AUTH, and composition before implementing or activating contributor preparation |
...
| [WS-POL-003](initiatives/WS-POL-003-unified-project-guide-compilation/STATUS.md) | Active delivery initiative; hidden compilation custody is merged through `03A` | Continue AUTH-12I activation, then authorized compilation persistence and later approval integration in dependency order |
| [WS-REV-001](initiatives/WS-REV-001-review-revision-lifecycle/STATUS.md) | Active delivery initiative; queue admission and reviewer-lease persistence are merged through `03A2` | Continue packet, review, decision, revision, recovery, and release chunks after named ART, AUTH, and CON gates |
| [WS-QUAL-002](initiatives/WS-QUAL-002-behavior-ownership-catalogue/STATUS.md) | Active quality initiative; catalogue foundation `01` is merged | Add context evidence, then populate subsystem ownership before considering a changed-line-aware mutation gate |
