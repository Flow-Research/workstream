...
The [ARCH current dependency contract](../../WS-ARCH-001/planning/PLAN.md#current-dependency-contract)
coordinates the path through `allow_review`; the phase semantics below govern
POL work. Pending child contracts are updated with it. Completed records remain
evidence, not a second active queue.
...
Work evaluation outside the intake contract belongs to supported post-submit
implementations or explicitly designated human-review requirements. A model
judge may produce checker evidence; it is not the authorized human product
review. Claiming, assignment, deadlines, routing, revision, acceptance,
...
also be registered for that stage, closed-schema, bounded, credential-safe,
and implemented by trusted code. Current enabled primitives are deterministic;
that current limitation is not a permanent ban on model/agent work evaluation.
A new evaluator needs ARCH-04A's canonical capability implementation and
conformance proof before it can appear in a setup snapshot. Model-based
evaluation uses a typed registered adapter, locked requirements and evaluator
identity, strict result/evidence validation and bounded provider access; no
arbitrary project code, shell, tools or unrestricted network. Its timeout and
resource budget are catalogue-owned and enforced. Deterministic policy
compilation does not promise deterministic model judgments.
...
8. validate typed configuration and the registered execution-method constraints,
...
POL-04A2's closed setup row and finalization receipt are immutable. POL-05A
owns downstream pre-policy approval operations; POL-06A owns post-policy
projection/approval/correction operations. Link each to the exact finalization,
compilation and upstream policy identities. Do not reopen the finalized setup
row, repurpose its output fields, or let AUTH/live-cutover code invent storage.
...
Activation also requires one validated ContributionPolicyVersion with both
submitter/reviewer rules, and complete locked review/revision policy
configuration. It checks selected checker implementation/configuration support
from ARCH-04A/POL-07, not a Task, Submission, completed CheckerRun or REV runtime.
Task and post-submit execution prerequisites follow activation, not vice versa.
