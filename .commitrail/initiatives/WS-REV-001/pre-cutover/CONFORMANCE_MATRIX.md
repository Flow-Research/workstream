...
| Revision context | 09A1-A5, 09B | Review-rooted preparation consumes owner-supplied immutable Submission/current-context facts; complete guide/policy context kept/rebased/blocked atomically, including prior/next submitter ContributionPolicyVersion; exact head acknowledgement; one winner per head; replacement successor; completed lease/contribution/award history immutable; checker path bypasses human preparation | Human context display, checker rerun, prior-reviewer preference, resolution, final decision; checker correction returns open |
...
| Artifact packet access | 03B, 07A, 11D | Active-exact-lease packet bytes; metadata-only history; no reviewer/response evidence candidate/finalize path; no raw store/provider access | Local/MinIO/S3 packet-owner conformance plus outage/integrity no-adverse-outcome drill |
