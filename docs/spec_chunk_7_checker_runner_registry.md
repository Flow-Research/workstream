...
Pre-submit intake runs through hidden ART ZIP preparation and the internal
`evaluate_pre_submission` phase command. POL-07B removes the former draft JSON
precheck route, request/response schemas and exclusive runner. No public checker
endpoint accepts a draft packet as intake proof. ART retains bounded results;
the hidden preparation route returns `pre_submission_checker_failed` on blocking
failure. Structured public feedback remains part of the later intake cutover.
...
Blocking pre-submit failures occur before durable checker runs exist. Hidden
ZIP preparation reports `pre_submission_checker_failed`; it creates no Submission
or durable post-submit run. ART retains bounded pass/fail/warning evidence.
These results never use review decision values: `accept`, `needs_revision`, or
`reject`. Structured public feedback remains pending.
...
- hidden pre-submit ZIP checks retain ART evidence without durable post-submit checker rows
- blocking preparation returns `pre_submission_checker_failed`, creates no submission row or version, and does not transition the task or publish a submission-created event
- no standalone JSON precheck route or schema remains
