...
- Committed original-document readiness automatically runs one unified compilation
  ...
  Submission. Public activation wiring and public TASK queue access, ARCH-03C2 atomic assignment-invalidation producer wiring and first handler registration remain pending.
...
- A claim starts a project-policy lease using PostgreSQL time. Contributor skip
  or expiry releases only unsubmitted work; reclaim creates a new assignment.
  Foreground submission rejects expired or replaced assignments even before
  cleanup runs, and authorized own-history reads explain expiry.
