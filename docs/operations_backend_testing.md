...
uses six independent matrix jobs, one per semantic lane, with a 20-minute lane
...
The required GitHub check remains `Backend / test`. Six matrix jobs each own a
...
accepts exactly the six declared lane directories,
...
and coverage-file SHA-256 digests. Only then are exactly six regular,
...
records the exact head, canonical node count, six lane results, elapsed time,
