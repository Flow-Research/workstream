...
verification, scoped original-guide access, submission/checker binding, Operator observability, and
...
buffer must match the original's SHA-256 and size immediately before it is yielded.
No guide-source binding or extraction materialization request exists in the
...
and generated byte source. The Submission binding is instead the terminal result of the typed
`SubmissionAdmissionConsumptionPort`: it consumes one ready admission against
the exact TASK-supplied Submission identity and locked lineage. The remaining
checker-output binding request contains its required verified content ID and
exact CheckerRun selector. Each typed method fixes its own authority; no request
selects the action.
...
confirmed observation after acknowledgement loss, establishes that original's
readiness. Once every assigned original is committed, the existing setup
generation can be queued. Shared submission verification is a separate workflow.

Current guide inputs are document uploads plus ordinary task-example text stored
with guide metadata in PostgreSQL. URL fetching and durable URL source selectors
are unavailable. Any future external-source adapter requires its own approved
scope and retrieval controls; it is not a fallback for current guide input.
Retained binding/extraction evidence is read-only and cannot drive new setup.
...
producer, so sharing content with a guide does not disable submission checking.
...
idempotency key, required project/task IDs, optional submission ID, and expected
source-job CAS version. A submission ID remains optional because submission-bundle
verification precedes Submission creation. The transaction creates
...
it does not mark a shared replica verified for submissions. Once all exact
snapshot items are committed, Workstream queues the same setup generation for
one unified Celery compilation. Incomplete or uncertain storage never triggers
inference. Recovery of a committed upload rechecks current project, guide,
snapshot and generation before publishing that continuation.

The runtime receives an immutable manifest with opaque document handles. Its
only source tool opens an assigned handle through a fixed guide-reader service.
Before provider access, ART locks and checks the draft guide, current snapshot,
setup generation, fenced attempt, exact ingest/put/receipt, content, replica and
storage namespace. It stages and verifies the complete original through
`ArtifactScratchManager` before yielding bytes. Forged, foreign, stale, expired
or closed grants deny without reading an object. Neither model nor provider
workspace receives S3 credentials, bucket listing, object keys or URLs.

The OpenAI Agents SDK adapter stages opened originals into a fresh bounded
provider workspace with networking disabled. The agent can inspect files in
multiple tool turns rather than receiving all extracted text in one prompt.
Runtime, model, instructions, tool bounds and timeouts are independently
configured and captured in immutable attempt evidence. The default model
request timeout is 300 seconds; the whole-run timeout is separate.

PROJECTS records allocation intent before each provider allocation and the
returned exact resource identity before the next call. Successful file opens
bind the exact document version, digest, manifest and fenced attempt. Validated
findings and policy proposals cite those document versions; ready proposals
must account for every assigned document. Known malformed structured output
ends as invalid-terminal evidence. Uncertain provider execution cannot be
repeated by redelivering the job.

Cleanup deletes only recorded provider resource IDs, including partial staging.
A bounded cleanup reconciler retries remaining known resources without model
inference or source access. Unknown allocation outcomes rely on configured
provider expiry; cleanup never discovers resources by listing. Scratch has
aggregate quotas and crash cleanup and is never durable artifact storage.

The setup result contains sufficiency findings and distinct pre-submission and
post-submission policy proposals. A blocked guide ends at findings; a ready
guide stops at draft output. Project Manager decisions and fresh manual reruns
belong to POL-05; post-submit projection belongs to POL-06. No setup finding
...
- the baseline's guide binding, format-classification, custody-incident and
  extraction tables remain retained evidence only. The original-document
  cutover rejects new binding/extraction writes and preserves existing rows;
- current guide custody binds exact project/guide/snapshot/item, committed
  original digest/size, and run generation. Task-example hash/count commit to
  immutable guide metadata. Current inference reads assigned originals through
  scoped grants, without creating guide bindings or extraction usages;
- snapshot declarations use sanitized `source_label`, fixed `document`/`upload`
  metadata and the `guide_source_snapshot.task_examples` manifest. Caller-owned
  content identity and private fetch/storage locators are excluded.
