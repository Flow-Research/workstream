...
async def test_locked_context_serializes_with_real_revocation(admin_access, auth_database_env, monkeypatch, kind, first):
    project, manager, task = await task_case(admin_access)
    grant = await grant_for(admin_access, project, kind)
    ...
    async def run(operation):
        async with db_session.get_session_factory()() as session:
            ...
            return await getattr(command, kind + "_locked_context")(project, task)
...
async def test_locked_context_lock_order_and_refresh(admin_access, auth_database_env, monkeypatch, kind, corrupt):
    ...
    for owner,method,label in ((TaskRepository,"get_task","task"),(TaskRepository,"lock_project_task","task"),
                              ...
                              (ProjectLockedPolicyRepository,"lock_locked_policy_context","projects")):
    ...
    async with factory() as reader, factory() as writer:
        ...
        original_body = stale.locked_post_submit_checker_policy_body.copy()
        if corrupt:
            changed.locked_post_submit_checker_policy_body = {**original_body, "blocking_severities": []}
        await writer.flush()
        ...
        try:
            ...
            assert "projects" not in order
            assert stale.locked_post_submit_checker_policy_body == original_body
        finally:
            await writer.commit()
        if corrupt:
        ...
        # Reload after rollback in the invalid-custody branch.
        await reader.refresh(stale)
        assert stale.title == "Refreshed instructions"
        assert stale.locked_post_submit_checker_policy_body == ({**original_body, "blocking_severities": []} if corrupt else original_body)
...
@pytest.mark.parametrize("kind", KINDS)
async def test_wrong_project_locked_context_does_not_wait(admin_access, kind):
    from app.modules.tasks.models import WorkstreamTask
    ...
    await grant_for(admin_access,requested_project,kind)
    async with db_session.get_session_factory()() as holder, holder.begin():
        ...
        response = await asyncio.wait_for(admin_access.signed.client.get(
            path(kind,requested_project,task),headers=admin_access.target.headers,
        ),timeout=10)
...
@pytest.mark.parametrize("kind", KINDS)
async def test_foreign_lock_probe_detects_task_only_lookup(admin_access, monkeypatch, kind):
    from app.modules.tasks.repository import TaskRepository
    ...
    with pytest.raises(TimeoutError):
        await test_wrong_project_locked_context_does_not_wait(admin_access, kind)
