...
async def _names(admin_url: str) -> set[str]:
    ...
    try:
        rows = await connection.fetch(
            "SELECT datname FROM pg_database WHERE datname LIKE 'workstream_test_%'"
        )
...
async def _roles(admin_url: str) -> set[str]:
    ...
    try:
        rows = await connection.fetch(
            "SELECT rolname FROM pg_roles WHERE rolname LIKE 'workstream_role_%'"
        )
...
def _mock_successful_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, drop) -> None:
    ...
    monkeypatch.setattr(
        ...
        "_urls",
        lambda _: (
            "workstream_test_012345abcdef",
            "workstream_role_012345abcdef",
            "password",
            "target",
        ),
    )
    ...
    monkeypatch.setattr(
        sys, "argv", [str(RUNNER), "--metadata-json", str(tmp_path / "db.json"), "--", "child"]
    )


def test_run_emits_secret_free_heartbeat(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
) -> None:
    """Long children expose liveness without exposing buffered child output."""
    ...
    class Process:
        ...
        def communicate(self, timeout=None):
            ...
            if len(self.timeouts) == 1:
                raise runner.TimeoutExpired("child", timeout)
            return b"private stdout", b"private stderr"

    process = Process()
    clock = iter((0.0, 0.0, 60.0, 60.0))
    monkeypatch.setattr(runner.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(runner.time, "monotonic", lambda: next(clock))

    result = runner._run(["child"], {}, 120.0)

    code, stdout, stderr = result
    assert code == 0
    assert stdout == b"private stdout"
    assert stderr == b"private stderr"
    assert process.timeouts == [60.0, 60.0]
...
@pytest.mark.parametrize(
    ("code", "expected"), [("raise SystemExit(7)", 7), ("import time; time.sleep(2)", 124)]
)
...
def test_runner_strips_parent_secrets_migrates_and_redacts_target(
    ...
    try:
        ...
        raise AssertionError("runner metadata must be valid JSON") from exc
...
def test_cleanup_terminates_only_owned_sessions_and_drops_role(child_drops_database: bool) -> None:
    ...
    async def exercise() -> None:
        ...
        if child_drops_database:
            controller = await asyncpg.connect(
                runner._asyncpg_url(target).rsplit("/", 1)[0] + "/postgres"
            )
        ...
        if child_drops_database:
            ...
            with pytest.raises(
                (asyncpg.PostgresError, asyncpg.InterfaceError, ConnectionResetError)
            ):
...
@pytest.mark.parametrize(
    ("name", "role"),
    [
        ("workstream_test_bad", "workstream_role_012345abcdef"),
        ("workstream_test_012345abcdef", "admin"),
    ],
)
...
def test_runner_concurrency_is_unique_and_interrupt_cleanup_is_exact(tmp_path: Path) -> None:
    ...
    for index in range(2):
        ...
        ignored = (
            "import signal; signal.signal(signal.SIGTERM, signal.SIG_IGN); " if index == 0 else ""
        )
        code = (
            ignored
            + f"import os,time,urllib.parse,pathlib; pathlib.Path({str(marker)!r}).write_text(urllib.parse.urlsplit(os.environ['WORKSTREAM_DATABASE_URL']).path[1:]); time.sleep(30)"
        )
        command = [
            sys.executable,
            str(RUNNER),
            "--metadata-json",
            str(tmp_path / f"db-{index}.json"),
            "--",
            sys.executable,
            "-c",
            code,
...
def test_collision_never_claims_or_drops_existing_database() -> None:
    ...
    async def exercise() -> None:
        ...
        await runner._create(admin, name, role, password)
        target = (
            "postgresql" + admin.removeprefix("postgresql+asyncpg").rsplit("/", 1)[0] + f"/{name}"
        )
...
@pytest.mark.parametrize(
    ("migration_code", "interrupted", "expected"), [(9, False, 2), (130, True, 130)]
)
def test_migration_failure_or_interrupt_cleans_without_starting_child(
    ...
    def migration(*_):
        setattr(runner, "INTERRUPTED", interrupted)
