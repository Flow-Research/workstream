...
async def test_project_repository_postgresql_locked_policy_state_matrix(
    ...
    async with db_session.get_session_factory()() as session:
        ...
        await session.execute(
            update(EffectiveProjectSubmissionArtifactPolicy)
            .where(EffectiveProjectSubmissionArtifactPolicy.id == str(request.effective_policy_id))
    ...
    async with db_session.get_session_factory()() as session:
        ...
        session.add(
            EffectiveProjectSubmissionArtifactPolicy(
                ...
                submission_artifact_policy_hash=(
                    original_effective.submission_artifact_policy_hash
                ),
                lifecycle_status="approved",
        ...
        await session.commit()
    async with db_session.get_session_factory()() as session:
        still_historical = await ProjectRepository(session).lock_locked_policy_context(request)
        assert still_historical.effective_policy_id == request.effective_policy_id
        assert still_historical.pre_submit_policy_id == request.pre_submit_policy_id
...
async def test_submission_artifact_policy_agent_service_commits_and_replays_without_io(
    ...
    guide_2 = await create_guide(project_client, project_2["id"], complete_guide_payload())
    snapshot_2 = await create_source_snapshot(project_client, project_2["id"], guide_2["id"])
    ...
    async with db_session.get_session_factory()() as session:
        setup_2 = await session.scalar(
            select(ProjectSetupRun).where(ProjectSetupRun.source_snapshot_id == snapshot_2["id"])
        )
...
def test_submission_artifact_policy_agent_final_custody_rejects_stale_output(
    ...
    with pytest.raises(SubmissionPolicyMutationConflict):
        SubmissionPolicyMutationService._require_exact_running_custody(setup, lineage, custody)
