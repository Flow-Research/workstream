...
def test_task_read_migration(isolated_database_env, migration_lock):
    with migration_lock():
        async def reset():
...
def test_task_read_migration_exact_pairs(isolated_database_env, migration_lock, remove_guard):
    with migration_lock():
        async def probe():
            connection = await asyncpg.connect(isolated_database_env.replace("+asyncpg", ""))
            try:
