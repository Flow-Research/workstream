...
from tests.contributions.policy_test_support import (
    complete_rules,
    create_request,
    service_fixture,
)
...
def recovered_event(fixture: SimpleNamespace, request: object) -> SimpleNamespace:
    action = (
        ...
        if isinstance(request, ContributionPolicyUpdateDraftRequest)
        else "contribution.policy.create_draft"
    )
...
@pytest.mark.asyncio
async def test_update_duplicate_returns_exact_immutable_original_result() -> None:
    fixture = service_fixture()
...
@pytest.mark.asyncio
async def test_update_duplicate_requires_current_read_authorization() -> None:
    fixture = service_fixture()
    request = update_request(fixture)
    fixture.repository.get_event_by_operation.return_value = recovered_event(fixture, request)
...
@pytest.mark.asyncio
async def test_update_duplicate_creates_no_second_mutation_effect() -> None:
    fixture = service_fixture()
    request = update_request(fixture)
    fixture.repository.get_event_by_operation.return_value = recovered_event(fixture, request)
...
@pytest.mark.asyncio
async def test_update_duplicate_digest_mismatch_is_concealed() -> None:
    fixture = service_fixture()
    request = update_request(fixture)
    event = recovered_event(fixture, request)
    event.request_digest = "sha256:" + "0" * 64
