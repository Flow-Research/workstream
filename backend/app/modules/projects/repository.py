...
class ProjectRepository:
    ...
    async def lock_locked_policy_context(
        ...
        """Lock and resolve exact historical PROJECT policy lineage.

        Args:
            request: Exact TASK-stamped PROJECT lineage selectors.

        Returns:
            Immutable canonical PROJECT facts from the locked rows.

        Raises:
            ProjectLockedPolicyContextUnavailable: When any selected row,
                lifecycle state, lineage link, or canonical hash has changed.
        """
        ...
        effective_policy = await lock_by_id(
            EffectiveProjectSubmissionArtifactPolicy,
            request.effective_policy_id,
        )
        pre_submit_policy = await lock_by_id(
            PreSubmitCheckerPolicy,
            request.pre_submit_policy_id,
        )
        if (
            project is None
        ...
        if not all(isinstance(value, Mapping) for value in policy_values):
            raise ProjectLockedPolicyContextUnavailable("project_locked_policy_context_changed")
        try:
            canonical_snapshot = CanonicalJsonObject.from_mapping(snapshot.manifest_json)
            canonical_effective_policy = CanonicalJsonObject.from_mapping(
                effective_policy.effective_policy
            )
            canonical_pre_submit_bundle = CanonicalJsonObject.from_mapping(
                pre_submit_policy.compiled_bundle
            )
        ...
        expected = (
            ...
            canonical_effective_policy.sha256 == request.effective_policy_hash,
            canonical_pre_submit_bundle.sha256 == request.pre_submit_policy_bundle_hash,
        )
        if not all(expected):
            raise ProjectLockedPolicyContextUnavailable("project_locked_policy_context_changed")
        try:
            return ProjectLockedPolicyContextFacts(
                ...
                effective_policy=canonical_effective_policy,
                ...
                compiled_pre_submit_bundle=canonical_pre_submit_bundle,
            )
        except (TypeError, ValueError) as exc:
            raise ProjectLockedPolicyContextUnavailable(
                "project_locked_policy_context_changed"
