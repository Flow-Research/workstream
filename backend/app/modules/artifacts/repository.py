...
class ArtifactRepository:
    ...
    async def get_guide_lineage(self, guide_source_item_id: str) -> GuideLineageFacts | None:
        ...
        lineage = (
            await self._session.execute(
                select(
                    GuideSourceSnapshotItem.id,
                    GuideSourceSnapshotItem.source_kind,
                    GuideSourceSnapshotItem.ingestion_adapter,
    ...
    @staticmethod
    def _current_verification_producer():
        return select(ArtifactPutAttempt.id).where(
            ArtifactPutAttempt.id == ArtifactVerificationJob.originating_put_attempt_id,
            ArtifactPutAttempt.producer_request_type.in_(VERIFICATION_PRODUCERS),
        ).exists()
        ...
        *,
        ...
        statement = (
            ...
            .where(
                ...
                self._current_verification_producer(),
                eligible,
            )
            .values(
                status="running",
