"""Admit only the exact task-evidence read authorization evidence pairs."""

from alembic import op
import sqlalchemy as sa

revision = "0005_task_evidence_authority"
down_revision = "0004_task_context_authority"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    addition = " OR ".join(
        ...
        for action, permission in (
            ("audit.task.evidence.read", "audit.read"),
        )
    )
    if definition.count(anchor) != 2:
        raise RuntimeError("task evidence authority constraint shape changed")
